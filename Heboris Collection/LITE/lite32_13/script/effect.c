...
void objectStatBlock(int no) {
	...
	if(objj[no] >= 10) {
		...
		if((obja[no] < 32) && (gameMode[player] == 6) && (tomoyo_timebonus[player]))
			BltRect(3, objx[no], objy[no] - obja[no], 212 + (objj[no] - 10) * 12, 64, 12, 7);
		...
		// 新TAP/TI
		BltRect(31 + objj[no], objx[no] - 44, objy[no] - 20 - (!breakgraphics) * 24, 
					(obja[no] % (8 - (2 * breakgraphics))) * 96, (obja[no] / (8 - (2 * breakgraphics))) * 96, 96, 96);
		if(obja[no] >= 36) obj[no] = 0;
	} else {
		// 旧TAP/TI
		BltRect(31, objx[no] - 12, objy[no] - 12, (obja[no] / 3) * 32, objj[no] * 32, 32, 32);
...
void objectAllCrear(int no) {
	...
	// BRAVO!
	BltRect(49, foffset[player] - 2, (objy[no] - 1) * 8, 100, 150 + 25 * (count % 4 / 2), 100, 25);
...
void objectHanabi(int no) {
	...
	// 96x96
	// 48frames
	BltRect(39 + objj[no], objx[no], objy[no], (obja[no] % 6) * 96, (obja[no] / 6) * 96, 96, 96);
	obja[no]++;
...
void objectplatina_erase(int no) {
	// 32x32
	// 60frames
	BltRect(10 + objj[no] - 10, objx[no] - 12, objy[no] - 12, (obja[no] % 10) * 32, (obja[no] / 10) * 32, 32, 32);
...
void objectNagareboshiC(int no) {
	BltRect(20, objx[no], objy[no], (obja[no] % 6) * 32, (obja[no] / 6) * 32, 32, 32);
	obja[no]++;
...
void objectGetmedal(int no) {
	...
	BltRectR(0, objx[no] - i, objy[no] - j, objv[no] * 16, objc[no] * 12, 16, 12, k, k);
...
void objectItemWarning(int no) {
	...
	BltRect(49, foffset[player], 84, 0, 150 + 25 * (count % 4 / 2), 100, 25);
	
	if(objj[no] >= 0){
		BltRect(46, (boffset[player] + 1) * 8, 116, (8 * objj[no]) + 56, 0, 8, 8);
		BltRect(79, (boffset[player] + 2) * 8, 112, 0, objj[no] * 16, 48, 16);
		BltRect(46, (boffset[player] + 8) * 8, 116, (8 * objj[no]) + 56, 0, 8, 8);
	}
...
void objectItemErase(int no) {
	BltRect(29, objx[no] - 36, objy[no] - 36, (obja[no] % 6) * 80, (obja[no] / 6) * 80, 80, 80);
