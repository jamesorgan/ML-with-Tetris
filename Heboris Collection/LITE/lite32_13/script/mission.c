...
void viewMissionFrame() {
	...
	// 枠
	BltFastRect(4, moffset, 112, 0, 64, 112, 112);
	...
	for(i = tmp; i < (tmp + 5); i++) {
		if((c_mission > i) || (ending[0] != 0)) {
			// クリア済み
			BltFastRect(4, moffset, 32 + (i - tmp) * 16, 0, 48, 96, 16);
			BltRect(3, moffset + 98, 34 + (i - tmp) * 16, 12, 191, 12, 12);
		} else if(c_mission == i) {
			// プレイ中
			BltFastRect(4, moffset, 32 + (i - tmp) * 16, 0, 32, 96, 16);
			BltRect(3, moffset + 98, 34 + (i - tmp) * 16, 0, 191, 12, 12);
		} else {
			// まだ未プレイ
			BltFastRect(4, moffset, 32 + (i - tmp) * 16, 0, 16, 96, 16);
			BltRect(3, moffset + 98, 34 + (i - tmp) * 16, 0, 191, 12, 12);
		}
...
void viewEraserLines() {
	...
	for(i = 0; i < eraser_now_lines; i++) {
		if(mission_type[c_mission] == 40)
			BltRect(4, (boffset[0] - 1) * 8, (eraser_lines[i] + 3) * 8, 0, 176 + eraser_cleared[i] * 8, 95, 8);
		else
			BltRect(4, (boffset[0] - 1) * 8, (eraser_lines[i] + 3) * 8, 0, eraser_cleared[i] * 8, 95, 8);
	}
}
