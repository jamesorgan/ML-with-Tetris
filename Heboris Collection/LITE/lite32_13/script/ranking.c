...
void RankingView() {
	...
	for(i = 0; i < 3; i++) {
		...
		// メダル
		if(rkst[j + i]) BltRect(0, 10, 50 + i * 32, 16, rkst[j + i] * 12, 16, 12);
		if(rkac[j + i]) BltRect(0, 30, 50 + i * 32,  0, rkac[j + i] * 12, 16, 12);
		if(rkco[j + i]) BltRect(0, 50, 50 + i * 32, 48, rkco[j + i] * 12, 16, 12);
		if(rksk[j + i]) BltRect(0, 70, 50 + i * 32, 32, rksk[j + i] * 12, 16, 12);
	}
}
