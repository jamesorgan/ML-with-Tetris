...
str		version = "LITE32.13 (07/07/04)";
...
void title() {
	...
	while(loopFlag) {
		...
		if(!title_mov_f) {	// 静止画
			...
			BltFastRect(8, 0, 0, (title_acnt / 10) * 320, (title_acnt % 10) * 240, 320, 240);
		...
		if(mode == 0) {
			// スタートボタンを押してください
			if(count % 80 < 40) {
				BltRect(49,53,154,0,204,213,19);
			}
...
void lastProc() {
	...
	if(demo) {
		...
		if(demokind) {
			if(count % 80 < 40) BltRect(49,53,154,0,204,213,19);
			if(maxPlay) BltRect(3, 98, 100 + (count % 40 > 20), 0, 160, 123, 31);
		} else {
			showControl();
		}
...
void staffRoll(int player) {
	if((ending[player] == 2) && (onRecord[player]) && (!pause[player])) {
		...
		// 描画する
		BltRect(81 + staff_type[player], foffset[player] + 8, 40, 0, staff_c[player] / 2, 80, 160);
	...
	if(tomoyo_dual_enable[player]) {
		...
		if((tomoyo_ehfinal_c[player] < 220) || (tomoyo_ehfinal_c[player] > 240)) {
			if(foffset[0] == v_foffset[0])
				BltFastRect(84, foffset[1] + ofs_x[0] + 8, 40 + ofs_y[0], 0, 0, 80, 160);
			else
				BltFastRect(24, foffset[1] + ofs_x[0] + 8, 40 + ofs_y[0], 0, 0, 80, 160);
		}
	}
...
void statReady(int player) {
	...
	if( (statc[player * 10] >= ready_start[player]) && (statc[player * 10] < ready_end[player]) ) {
		// READY
		BltRect(49, foffset[player] - 2, 91, 200,  0, 100, 50);
	} else if( (statc[player * 10] >= go_start[player]) && (statc[player * 10] < go_end[player]) ) {
		// GO
		BltRect(49, foffset[player] - 2, 92, 200, 50, 100, 50);
	}
		...
		if(fmirror_cnt[player] <= p_fmirror_timer) {
			if(fmirror_cnt[player] == -20) {
				...
				if( (fmirror_cnt[player] > 0) && (fmirror_cnt[player] % (p_fmirror_timer / 10) == 0) ) {
					...
					BltFast(17, 8 + ((10 - j - 1) * 8) + foffset[player], 40);
				}
			}
			fmirror_cnt[player]++;
			field_spin[player]++;
...
void statVersusWait(int player) {
	if(count % 60 > 30) {
		// WAIT
		BltRect(49, foffset[player] - 2, 91, 100, 0, 100, 50);
	}
...
void statGameOver2(int player) {
	...
	// GAME OVER
	BltRect(49, foffset[player] - 2, 95, 0, 0, 100, 50);
	...
	*/
	...
	lclear:
		...
		if((gameMode[player] == 8) && (mission_clear_anim == 0)) {
			// CLEAR!!
			BltRect(3, foffset[player] - 7, 96, 195, 230, 109, 20);
		           ...
		           ((gameMode[player] == 16) && (grade[player] >= 14)) )
		{
			...
			if(statc[player * 10] < 64) {
				...
				j = (36 * k / 64 - 36) / 2;
				k = 65536 * k / 64;
				BltRectR(3, foffset[player] + 8 - i, 102 - j, 208, 126, 80, 36, k, k);
			} else {
				BltRect(3, foffset[player] + 8, 102, 208, 126, 80, 36);
			}
		           ...
		           (gameMode[player] == 14) )
		{
			...
			if(statc[player * 10] < 64) {
				...
				j = (66 * k / 64 - 66) / 2;
				k = 65536 * k / 64;
				BltRectR(3, foffset[player] - 7 - i, 70 - j, 191, 163, 112, 66, k, k);
			} else {
				BltRect(3, foffset[player] - 7, 70, 191, 163, 112, 66);
			}
		} else if(gameMode[player] == 15) {
			// FLASHモード
			...
			if(statc[player * 10] <= 5){
				...
				j = (9 * k / 5 - 9) / 2;
				k = 65536 * k / 5;
				BltRectR(3, foffset[player] + 10 - i, 96 - j, 212, 73, 75, 9, k, k);
			} else {
				BltRect(3, foffset[player] + 10, 96, 212, 73, 75, 9);
			}
	...
	lretry:
		...
		if(gameMode[player] == 8) {
			// CLEAR
			BltRect(3, foffset[player] - 7, 96, 195, 250, 109, 20);
		} else {
			// EXCELLENT
			if(statc[player * 10] <= 5){
				k = 10 - statc[player * 10];
...
void statFieldEditor(int player) {
	...
	// カーソル枠描画
	if(count % 16 < 8)
		BltFastRect(30, (statc[3 + player * 10] + boffset[player]) * 8 - 1, (statc[4 + player * 10] + 3) * 8 - 1, 20, 0, 10, 10);
	else
		BltFastRect(30, (statc[3 + player * 10] + boffset[player]) * 8 - 1, (statc[4 + player * 10] + 3) * 8 - 1, 10, 0, 10, 10);
...
void statExamination(int player) {
	...
	if(statc[player * 10 + 1] == 0) {
		...
		// 試験指令
			...
			if(statc[player * 10 + 0] < 170) {
				BltRectR(49, foffset[player] - 2 - ((100 * 22 / 12 - 100) / 2), 80, 200, 100 + 25 * (count % 4 / 2), 100, 25, 65536 * 22 / 12, 65536 * 22 / 12);
				...
				BltRectR(49, foffset[player] - 2 - i, 48 + 192 - statc[player * 10 + 0] , 200, 100 + 25 * (count % 4 / 2), 100, 25, k, k);
			} else {
				BltRect(49, foffset[player] - 2, 48, 200, 100 + 25 * (count % 4 / 2), 100, 25);
			}
			
			// 目標段位の表示
			if(statc[player * 10 + 0] > 210) {
				if(!english) BltRect(49, foffset[player] - 2, 74, 200, 180, 100, 20);
	...
	// 結果発表
		...
		// 結果発表
			...
			if(!english) BltRect(49, foffset[player] - 2, 48, 200, 150, 100, 30);
			...
			if((statc[player * 10 + 0] > 150) && (statc[player * 10 + 0] < 350)) {
				...
				if((statc[player * 10 + 0] % 10 < 5) || (statc[player * 10 + 0] >= 320)) {
					// 不合格
					if(!english) BltRect(49, foffset[player] + 13, 162, 215, 250, 70, 25);
					else printFont(boffset[player] + 2, 20, "FAILED", 0);
				} else {
					// 合格
					if(!english) BltRect(49, foffset[player] + 13, 162, 215, 200, 70, 25);
			...
			if(statc[player * 10 + 0] >= 350) {
				// 合格
				if(grade[player] >= exam_grade[player]) {
					if(!english) BltRect(49, foffset[player] + 13, 162, 215, 200 + 25 * (count % 4 / 2), 70, 25);
				...
				// 不合格
					...
					if(!english) BltRect(49, foffset[player] + 13, 162, 215, 250 + 25 * (count % 4 / 2), 70, 25);
...
void winner() {
	...
	for(player = 0; player < 2; player++) {
		...
		BltRect(3, foffset[player] + 6, winu, 96, obj * 25, 86, 24);
		...
		if((i < 72) || (i > 74) || (!background_compatibility)) {
			...
			for(movframe = 0; movframe < max; movframe++) {
				...
				for(j = 0; j <= players; j++) {
					...
					BlendBltRect(24, (tmp_offset + 8) + tmp1, 40 + tmp2, 0, 0, 80, 160, tmptr, tmptr, tmptr, fldtr, fldtr, fldtr);
					
					if(top_frame) {
						BlendBltRect(3, (tmp_offset - 15) + tmp1, tmp2, 127, 268, 126, 35, tmptr, tmptr, tmptr, fldtr, fldtr, fldtr);
					} else {
						// グラデーション部分をなめらかに#1.60c7p9ex
						BlendBltRect(3, tmp_offset + tmp1, tmp2, 101, 128, 100 - 20, 32, tmptr, tmptr, tmptr, fldtr, fldtr, fldtr);
						for(k = 0; k < 20; k++) {
							tr = tmptr * (19 - k) / 20;
							tr = 256 - tr;
							BlendBltRect(3, tmp_offset + (80 + k) + tmp1, tmp2, 101 + (80 + k), 128, 1, 32, 256 - tr, 256 - tr, 256 - tr, tr, tr, tr);
						}
					}
				}
			}
...
void spriteTime() {
	...
	if(screenMode >= 2) {
		...
		BltFastRectR(9, 0, 0, 0, 0, 320, 240, 65536 * 2, 65536 * 2);
		SwapToSecondary(9);
		BltFast(9, 0, 0);
	}
