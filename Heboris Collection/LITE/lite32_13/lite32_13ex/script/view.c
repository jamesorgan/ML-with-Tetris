...
void showControl() {
	...
	// レバーのカバー部分
	ExBltRect(3, coffset + 6, 126, 198, 38, 20, 20);

	// ボタン
	ExBltRect(3, coffset + 30, 126, 224 + getPressState(0, 4) * 32, 0, 32, 32);
	ExBltRect(3, coffset + 50, 132, 224 + getPressState(0, 5) * 32, 0, 32, 32);
	ExBltRect(3, coffset + 70, 138, 224 + getPressState(0, 6) * 32, 0, 32, 32);
	ExBltRect(3, coffset + 30, 144, 224 + getPressState(0, 7) * 32, 0, 32, 32);
	...
	ExBltRect(3, coffset + zx * bai, 120 + zy * bai, 192, 0, 32, 32);
}
...
void drawGradeImage(int x, int y, int number, int type) {
	if(type == 0) {
		...
		if(number == 32)				// GM
			ExBltRect(21, x, y, 270, ((count % 4 / 2) * 216) + 120, 90, 48);
		else if(number >= 27)			// M～MM
			ExBltRect(21, x, y, 270, ((count % 4 / 2) * 216) + ((number - 27) * 24), 90, 24);
		else if(number >= 18)			// m1～m9
			ExBltRect(21, x, y, 180, ((count % 4 / 2) * 216) + ((number - 18) * 24), 90, 24);
		else if(number >= 9)			// S1～S9
			ExBltRect(21, x, y,  90, ((count % 4 / 2) * 216) + ((number -  9) * 24), 90, 24);
		else							// 9～1
			ExBltRect(21, x, y,   0, ((count % 4 / 2) * 216) + (number * 24), 90, 24);
	} else if(type == 3) {
		// MASTER-EASY
		if(number <= 8)					// 9～1
			ExBltRect(21, x, y,   0, ((count % 4 / 2) * 216) + (number * 24), 90, 24);
		...
		if(number == 0)					// 1
			ExBltRect(21, x, y,   0, ((count % 4 / 2) * 216) + 192, 90, 24);
		else if(number < 10)			// S1～S9
			ExBltRect(21, x, y,  90, ((count % 4 / 2) * 216) + ((number -  1) * 24), 90, 24);
		else if(number < 14)			// S10～S13
			ExBltRect(21, x, y, 360, ((count % 4 / 2) * 216) + ((number - 10) * 24), 90, 24);
		else if(type == 1)				// GM
			ExBltRect(21, x, y, 270, ((count % 4 / 2) * 216) + 120, 90, 48);
		else							// GOD(ネ申)
			ExBltRect(21, x, y, 360, ((count % 4 / 2) * 216) + 96, 90, 72);
	}
}
...
//▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲△▲
void viewDarkTitle() {
	if(!skip_viewbg) ExBltFast(51, 0, 0);
}
...
void viewBack() {
	...
	if(back_mov_f[no] == 0) {
		// 静止画
		ExBltFastRect(60 + no, 0, 0, 0, 0, 320, 240);
	} else {
		// 動画
		ExBltFastRect(60 + no, 0, 0, (back_acnt / 10) * 320, (back_acnt % 10) * 240, 320, 240);
	...
	if(bgfadesw && !demo) {
		if(enable_bgfade) {
			...
			if(bgfadelv < 14) {
				...
				for(i = 0; i < 5; i++) {
					for(j = 0; j < 5; j++) {
						ExBltRect(50, i * 64, j * 48, 0, bgfadelv * 48, 64, 48);
					}
				}
...
void viewScoreNormal(int player) {
	...
	// 横線
	ExBltRect(3, (soffset[player] * 8) + 2, 178, 38, 34, 24, 3);
	if(sp[player] <= 600) {
		ExBltFastRect(3, (soffset[player] * 8) + 3, 179, 60 - ((sp[player] * 24) / 660), 37, 22, 1);
	} else {
		ExBltFastRect(3, (soffset[player] * 8) + 3, 179, 38, 37, 22, 1);
	}
	...
	if((medal_st[player] != 0) && (medal_visible_st[player] != 0))
		ExBltRect(0, soffset[player] * 8 + 2     , 116, 16, medal_st[player] * 12, 16, 12);
	
	if((medal_ac[player] != 0) && (medal_visible_ac[player] != 0))
		ExBltRect(0, soffset[player] * 8 + 2 + 16, 116,  0, medal_ac[player] * 12, 16, 12);
	
	if((medal_co[player] != 0) && (medal_visible_co[player] != 0))
		ExBltRect(0, soffset[player] * 8 + 2     , 129, 48, medal_co[player] * 12, 16, 12);
	
	if((medal_sk[player] != 0) && (medal_visible_sk[player] != 0))
		ExBltRect(0, soffset[player] * 8 + 2 + 16, 129, 32, medal_sk[player] * 12, 16, 12);
...
void viewScoreTomoyoDual(int player) {
	...
	// 横線
	ExBltRect(3,146,178,38,34,24,3);
	if(sp[player] <= 1149) {
		ExBltFastRect(3, 147, 179, 60 - ((sp[player] * 24) / 660), 37, 22, 1);
	} else {
		ExBltFastRect(3, 147, 179, 38, 37, 22, 1);
	}
...
void viewScoreAce(int player) {
	...
	// 横線
	ExBltRect(3, ((soffset[player] + ace_frame) * 8) + 2, 178, 38, 34, 24, 3);
	ExBltFastRect(3, ((soffset[player] + ace_frame) * 8) + 3, 179, 60, 37, 22, 1);
	...
	if( ((gameMode[player] == 8) && (!mission_time[c_mission])) || ((gameMode[player] == 5) && (!p_timelimit)) ) {
		ExBltFastRect(49, foffset[player] - 1, 216, 1, 229 + english * 18, 98, 16);
	} else {
		if(ltime[player] <= 10 * 60)
			color2 = (ltime[player] != 0) * 2;
		else
...
void viewScoreVersus(int player) {
	...
	if(item_mode[player]) {
		...
		// アイテムゲージ
		ExBltRect(78, 114 + player * 80, 80, 0, 0, 12, 75);
		ExBltRect(78, 114 + player * 80,
		        153 - ((70 * i) / item_interval), 12 + ismiss[player] * 24,
		         72 - ((70 * i) / item_interval), 12, ((70 * i) / item_interval) );
		
		// ゲージのフラッシュ
		if(itemappear_c[player]) {
			if(itemappear_c[player] % 2 == 0) ExBltRect(78, 114 + player * 80, 80, 24, 0, 12, 75);
			itemappear_c[player]--;
		}
	...
	if(!vs_goal_type) {
		...
		// 横線
		ExBltRect(3,114 + 68 * player,178,38,34,24,3);
		if(sp[player] <= 1149) {
			ExBltFastRect(3, 115 + 68 * player, 179, 60 - ((sp[player] * 24) / 660), 37, 22, 1);
		} else {
			ExBltFastRect(3, 115 + 68 * player, 179, 38, 37, 22, 1);
		}
	...
	/* ライン */
		...
		// 横線
		ExBltRect(3,114 + 68 * player,178,38,34,24,3);
		ExBltFastRect(3, 115 + 68 * player, 179, 60, 37, 22, 1);
...
void viewScoreStandard(int player) {
	...
	if((timeattack[player] == 0) || (timeattack[player] == 2)) {
		...
		// 横線
		ExBltRect(3, (soffset[player] * 8) + 2, 178, 38, 34, 24, 3);
		ExBltFastRect(3, (soffset[player] * 8) + 3, 179, 60, 37, 22, 1);
...
void viewFieldHold(int player) {
	...
	if(heboGB[pl2] || disable_hold || isholdlock[pl2])
		ExBltRect(3, foffset[player] - 2, 5, 275, 275, 27, 27);
}
...
void viewFieldFrame(int player, int allview) {
	...
	// 上部のグラフィックを描画
	if(top_frame) {
		ExBltRect(3, foffset[player] - 15, 0, 0, 268, 126, 35);
	} else {
		ExBltRect(3, foffset[player], 0, 0, 128, 100, 32);
	}
	ExBltRect(3, foffset[player] + 38, 0, 128, 112, 26, 7);
	ExBltRect(3, foffset[player]     , 4, 128, 119, 26, 7);
	...
	if(block_shadow && ace && ace_frame) {
		if(allview) {
			ExBltRect(2, foffset[player] - 8 + ofs_x[player] + 2 - (4 * IsVs2p(player)), 32 + ofs_y[player], 960, 184, 112, 184);
		} else {
			ExBltRect(2, foffset[player] - 8 + ofs_x[player] + 2 - (4 * IsVs2p(player)), 32 + ofs_y[player], 960, 184, 112, 8);
		}
	}
	...
	if( (field_spin[player] < 0) || (field_spin[player] > 19) ) {
		...
		if(block_shadow && allview && !iscolor[player]) {
			ExBltRect(2, foffset[player] + ofs_x[player] + 2 - (4 * IsVs2p(player)), 32 + ofs_y[player], 96 * j, 184, 96, 184);
		}
		ExBltRect(2, foffset[player] + ofs_x[player], 32 + ofs_y[player], 96 * j, 0, 96, 8 + allview * 176);
	} else if(allview) {
		// 回転してる場合
		if(j == 5) tmp = 19;
		else if(j == 2) tmp = 77;
		...
		ExBltRect(tmp, foffset[player] + ofs_x[player], 17 + ofs_y[player], 96 * field_spin[player], 0, 96, 206);
	}
	
	// ACEモードの枠の出っ張り
	if(ace && ace_frame) {
		if(allview) {
			ExBltRect(2, foffset[player] - 8 + ofs_x[player], 32 + ofs_y[player], 960, 0, 112, 184);
		} else {
			ExBltRect(2, foffset[player] - 8 + ofs_x[player], 32 + ofs_y[player], 960, 0, 112, 8);
		}
	}
	...
	// アイテム名
	if(item_name[player]) ExBltRect(79, foffset[player] + 65, 0, 0, (item_name[player] - 1) * 16, 48, 16);
}
...
void viewFieldBackground(int player) {
	...
	if( ((iscolor[player]) || (heboGB[player] == 2)) && (!skip_viewbg) )
		ExBltFastRect(24, foffset[player] + ofs_x[player] + 8, 40 + ofs_y[player], 0, 0, 80, 160);
	
	// オールドスタイル
	if(heboGB[player] == 1) {
		ExBltFastRect(84, foffset[player] + ofs_x[player] + 8, 32 + ofs_y[player], 0, 0, 80, 8);
		ExBltFastRect(84, foffset[player] + ofs_x[player] + 8, 40 + ofs_y[player], 0, 0, 80, 160);
	}
				...
				if(fi != -1) {
					if(!b) {
						...
						if(getFieldBlockTomoyoRiseEditor(player, k - 1, j) == 0)
							ExBltRect(30, (k + boffset[player]) * 8 - 1, (j + 3 + 2) * 8 - 1, 54, 0, 10, 10);
						
						// 右
						if(getFieldBlockTomoyoRiseEditor(player, k + 1, j) == 0)
							ExBltRect(30, (k + boffset[player]) * 8 - 1, (j + 3 + 2) * 8 - 1, 64, 0, 10, 10);
						
						// 上
						if(getFieldBlockTomoyoRiseEditor(player, k, j - 1) == 0)
							ExBltRect(30, (k + boffset[player]) * 8 - 1, (j + 3 + 2) * 8 - 1, 74, 0, 10, 10);
						
						// 下
						if(getFieldBlockTomoyoRiseEditor(player, k, j + 1) == 0)
							ExBltRect(30, (k + boffset[player]) * 8 - 1, (j + 3 + 2) * 8 - 1, 84, 0, 10, 10);
					}
				}
			}
}
					...
					if(!b) {
						...
						if(i) {
							...
							if(getFieldBlock(player, k - 1, j) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] - 1, (j + 3) * 8 + ofs_y[player] - 1, 54, add, 10, 10);
							
							// 右
							if(getFieldBlock(player, k + 1, j) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] - 1, (j + 3) * 8 + ofs_y[player] - 1, 64, add, 10, 10);
							
							// 上
							if(getFieldBlock(player, k, j - 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] - 1, (j + 3) * 8 + ofs_y[player] - 1, 74, add, 10, 10);
							
							// 下
							if(getFieldBlock(player, k, j + 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] - 1, (j + 3) * 8 + ofs_y[player] - 1, 84, add, 10, 10);
							
							// 左上
							if(getFieldBlock(player, k - 1, j - 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player], (j + 3) * 8 + ofs_y[player], 55, add + 1, 1, 1);
							
							// 右上
							if(getFieldBlock(player, k + 1, j - 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] + 7, (j + 3) * 8 + ofs_y[player], 55, add + 1, 1, 1);
							
							// 左下
							if(getFieldBlock(player, k - 1, j + 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player], (j + 3) * 8 + ofs_y[player] + 7, 55, add + 1, 1, 1);
							
							// 右下
							if(getFieldBlock(player, k + 1, j + 1) == 0)
								ExBltRect(30, (k + boffset[player]) * 8 + ofs_x[player] + 7, (j + 3) * 8 + ofs_y[player] + 7, 55, add + 1, 1, 1);
						}
					}
				}
			}
...
void printScoreFont(int fontX, int fontY, str *fontStr, int fontColor) {
	...
	for(i = 0; i < stringLength; i++) {
		...
		sx = ( (stringChar - 32) % 32 ) * 8;
		sy = ( (stringChar - 32) / 32 ) * 8 + fontColor * 24;
		ExBltRect(1, (fontX + i) * 8 + (block_shadow * 2), fontY * 8, sx, sy, 8, 8);
	}
}
...
void printBIGFont(int fontX, int fontY, str *fontStr, int fontColor) {
	...
	for(i = 0; i < stringLength; i++) {
		...
		if(sx >= 0) ExBltRect(22, fontX + 12 * i, fontY, sx, sy, 12, 16);
	}
}
		...
		for(i = 0; i < 4; i++) {
			...
			if(j == 0) {
				if((dhold[player]) && (hold_used[player] >= 2) && (holdframetype == 0))
					ExBltFastRect(30, bx2 - 1, by2 - 1 + offset, 10 * ((count % 16 < 8) + 1), 0, 10, 10);
			} else {
				if(flash == 10) {			// 固定時の点滅
					if(bright >= 17)
...
void drawBigBlock(int bx1, int by1, int kind, int rotate, int color, int bright, int offset, int flash, int player) {
	...
	for(j = (holdframetype != 0); j <= ((flash == 10) || (flash < 2)); j++) {
		for(i = 0; i < 4; i++) {
			...
			if(j == 0) {
				if((dhold[player]) && (hold_used[player] >= 2) && (holdframetype == 0))
					ExBltFastRect(30, bx2 - 1, by2 - 1 + offset, 18 * ((count % 16 < 8) * 2), 10, 18, 18);
			} else {
				if(flash == 10) {			// 固定時の点滅
					if(bright >= 7)
...
void drawNaviBlock(int bx1, int by1, int kind, int rotate, int color, int bright, int player) {
	...
	if(bright < 8) {
		for(i = 0; i < 4; i++) {
			...
			bx2 = data[0] * 8;
			by2 = data[1] * 8;
			ExBltRect(76, bx2, by2, color * 8, bright * 8, 8, 8);
		}
	}
}
...
void drawSingleBlock(int player, int bx2, int by2, int offset, int color, int bright, int size, int bl_on, int bl1, int bl2)
{
	...
	if(size == 0) {
		...
		if(!bl_on)
			ExBltFastRect(plane, bx2, by2 + offset, tmp, bright * 8, 8, 8);
		else
			BlendExBltRect(plane, bx2, by2 + offset, tmp, bright * 8, 8, 8, bl1, bl1, bl1, bl2, bl2, bl2);
	}
	...
	if(size == 1) {
		...
		if(!bl_on)
			ExBltFastRect(plane, bx2, by2 + offset, tmp, bright * 16, 16, 16);
		else
			BlendExBltRect(plane, bx2, by2 + offset, tmp, bright * 16, 16, 16, bl1, bl1, bl1, bl2, bl2, bl2);
	}
	...
	if(size == 2) {
		...
		if(!bl_on)
			ExBltFastRect(plane, bx2, by2 + offset, tmp, bright * 4, 4, 4);
		else
			BlendExBltRect(plane, bx2, by2 + offset, tmp, bright * 4, 4, 4, bl1, bl1, bl1, bl2, bl2, bl2);
	}
}
...
void drawSingleBlockR(int player, int bx2, int by2, int color, int zoom) {
	...
	ExBltFastRectR(plane, bx2, by2, tmp, 0, 8, 8, zoom, zoom);
}
...
void drawBlockLine(int rule, int bx2, int by2, int kind, int rotate, int index, int bright, int offset, int size) {
	...
	if(!size) {
		...
		if(BitTest(linkdata, 0) == 0)
			ExBltRect(30, bx2 - 1, by2 - 1 + offset, 74, bright * 10, 10, 10);
		
		// 下
		if(BitTest(linkdata, 1) == 0)
			ExBltRect(30, bx2 - 1, by2 - 1 + offset, 84, bright * 10, 10, 10);
		
		// 左
		if(BitTest(linkdata, 2) == 0)
			ExBltRect(30, bx2 - 1, by2 - 1 + offset, 54, bright * 10, 10, 10);
		
		// 右
		if(BitTest(linkdata, 3) == 0)
			ExBltRect(30, bx2 - 1, by2 - 1 + offset, 64, bright * 10, 10, 10);
		
		// 左上
		if((BitTest(linkdata, 2) != 0) && (BitTest(linkdata, 0) != 0) && (kind != 2))
			ExBltRect(30, bx2, by2 + offset, 55, bright * 10 + 1, 1, 1);
		
		// 右上
		if((BitTest(linkdata, 3) != 0) && (BitTest(linkdata, 0) != 0) && (kind != 2))
			ExBltRect(30, bx2 + 7, by2 + offset, 55, bright * 10 + 1, 1, 1);
		
		// 左下
		if((BitTest(linkdata, 2) != 0) && (BitTest(linkdata, 1) != 0) && (kind != 2))
			ExBltRect(30, bx2, by2 + offset + 7, 55, bright * 10 + 1, 1, 1);
		
		// 右下
		if((BitTest(linkdata, 3) != 0) && (BitTest(linkdata, 1) != 0) && (kind != 2))
			ExBltRect(30, bx2 + 7, by2 + offset + 7, 55, bright * 10 + 1, 1, 1);
	} else {
		// BIGサイズ
		// 上
		if(BitTest(linkdata, 0) == 0)
			ExBltRect(30, bx2 - 2, by2 - 2 + offset, 133, bright * 20, 20, 20);
		
		// 下
		if(BitTest(linkdata, 1) == 0)
			ExBltRect(30, bx2 - 2, by2 - 2 + offset, 153, bright * 20, 20, 20);
		
		// 左
		if(BitTest(linkdata, 2) == 0)
			ExBltRect(30, bx2 - 2, by2 - 2 + offset, 94, bright * 20, 20, 20);
		
		// 右
		if(BitTest(linkdata, 3) == 0)
			ExBltRect(30, bx2 - 2, by2 - 2 + offset, 113, bright * 20, 20, 20);
		
		// 左上
		if((BitTest(linkdata, 2) != 0) && (BitTest(linkdata, 0) != 0) && (kind != 2))
			ExBltRect(30, bx2 - 1, by2 + offset, 55, bright * 10 + 1, 1, 1);
		
		// 右上
		if((BitTest(linkdata, 3) != 0) && (BitTest(linkdata, 0) != 0) && (kind != 2))
			ExBltRect(30, bx2 + 15, by2 + offset, 55, bright * 10 + 1, 1, 1);
		
		// 左下
		if((BitTest(linkdata, 2) != 0) && (BitTest(linkdata, 1) != 0) && (kind != 2))
			ExBltRect(30, bx2 - 1, by2 + offset + 15, 55, bright * 10 + 1, 1, 1);
		
		// 右下
		if((BitTest(linkdata, 3) != 0) && (BitTest(linkdata, 1) != 0) && (kind != 2))
			ExBltRect(30, bx2 + 15, by2 + offset + 15, 55, bright * 10 + 1, 1, 1);
	}
}
...
void drawSingleBlockShadow(int player, int bx2, int by2, int offset, int kind, int rotate, int color, int bright, int index, int size) {
	...
	if(size == 0) {
		...
		ExBltFastRect(83, bx2 + tmp2, by2 + offset, tmp, bright * 8, 2, 8);
	}
	...
	if(size == 1) {
		...
		ExBltFastRect(83, bx2 + tmp2, by2 + offset, tmp, 136 + bright * 16, 4, 16);
	}
	...
	if(size == 2) {
		...
		ExBltFastRect(83, bx2 + tmp2, by2 + offset, tmp, 264, 1, 4);
	}
}
