...
int ST_RankingCheck(int player, int rmode, int section,int enable_grade) {
	...
	tmp = ST_rankingGet(player,rmode,enable_grade[player]);
...
void ST_RankingUpdate(int player, int rmode, int end,int enable_grade) {
	...
	tmp4 = ST_rankingGet(player,rmode,enable_grade[player]);
...
int ST_rankingGet(int player,int rmode,int enable_grade){
	int	temp;
	if(rmode[player] == 0){//ビギナー
		temp = 0;
	}else if(rmode[player] == 1){//マスター
		if(enable_grade[player] !=4){
		temp = 4;
		}else if(enable_grade[player]==4){
		temp = 15;
		}
	}else if(rmode[player] == 2){//20G
		temp = 30;
	}else if(rmode[player] == 3){//デビル
		temp = 40;
	}else if(rmode[player] == 6){//TOMOYO
		temp =  60;
	}
	return temp;
}
