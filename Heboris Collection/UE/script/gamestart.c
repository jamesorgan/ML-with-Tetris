...
// 文字列定数
str		version = "C7V4EX YGS2K+ 07/08/02";	// 現在のスクリプトのバージョン(ver+date形式、1.60は除く)
...
void setNextBlockColors(int player, int first) {
	...
	if( first ) {
		...
		} else if(((p_over1000)&&(gameMode[player] == 5))||(monochrome_mode == 2)){	//即ブロックをを[]に
				if(rots[player] != 6){			//[]
					for(i = 0; i < 6; i++) {
						c_nblk[i + player * 6] = 8;
					}
		...
		if( (tc[player] >= over1000_start) ||(over1000_block[player])||((p_over1000)&&(gameMode[player] == 5))||(monochrome_mode == 2) ) {
			if(rots[player] != 6){	//[]
				for(i = tmp; i < 6; i++) {
					c_nblk[i + player * 6] = 8;
				}
...
void statNameEntry(int player) {
	...
	//モードと設定からカテゴリ設定
		...
		cat[player] = 0;
			...
			if(enable_grade==1){
			cat[player] = 1;
			}else if(enable_grade==2){
			cat[player] = 2;
			}else if(enable_grade==3){
			cat[player] = 3;
		...
		rank = RankingCheck2(cat[player], rots[player], temp, time[player], end_f[player]);
	}else{
			//モードと設定からカテゴリ設定
		if(gameMode[player]==0){
			if(novice_mode[player]){
