...
void viewFldBackground(void) {
	...
	for(i = 0; i <= maxPlay; i++) {	
		if((heboGB[i]==1) || (((tc[i] >= over1000_start) || (over1000_block) || ((p_over1000)&&(gameMode[i] == 5))) && (rots[i] == 6))){
		//フィールド背景を白く
...
void viewFldFrame(int uponly,int i) {
	int j, gm[2],waiting[2],ismission[2];
		...
		if(gameMode[i] == 10)//ORI
			...
			if(((tc[i] >= over1000_start) || (over1000_block) || ((p_over1000)&&(gameMode[i] == 5))) && (rots[i] == 6))
		...
		if((ismission[i]) && (disable_giji3D < 4))
			ExBltRect(2, 104 + 192 * i - 96 * maxPlay + ofs_x[i] + 2 - (4 * i), 32 + ofs_y[i], 960, 184, 112, 184);
