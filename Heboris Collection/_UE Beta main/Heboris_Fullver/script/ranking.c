...
str	rkname[ 6*14*2];			// 名前
int		rkdata[ 6*14*2];		// 花火／段位／ステージ／ライン
int		rktime[ 6*14*2];		// タイム
int		rkclear[ 6*14*2];		// 0=途中で窒息 1=ロール失敗 2=ロールクリア
int		rkother[ 6*14*2];		// その他（レベルやクリア率など）
int		rkrots[ 6*14*2];		// 回転ルール

int		rkac[ 6*14*2];		// ACメダル #LITE30.2
int		rkst[ 6*14*2];		// STメダル #LITE30.2
int		rksk[ 6*14*2];		// SKメダル #LITE30.2
int		rkco[ 6*14*2];		// COメダル #LITE30.2
int		rkre[ 6*14*2];		// REメダル #LITE30.2
...
//	if(rmode >= 8) return -1;
	...
	for(i = 0; i < 3; i++) {
		...
		if( (rdata > rkdata[j + i]) ||  //段位花火到達ステージライン
			...
			for(i = 0; i < 3; i++) {
				...
				if(rkpage == 0) {
					...
					if(rkpage == 4){//devil 
						...
						sprintf(string[0], "%d", rkdata[j + i]); //devilレベル
					}else{
						sprintf(string[0], "%d", rkdata[j + i]); //mas 20G段位
