...
void statReplaySave(int player) {
	...
	if((time2[player] > 72000 - 36000 * (gameMode[player] == modeNo_ver)) || (!replay_save[player])){	//リプレイ記録不可
		...
		if((time2[player] > 72000) && (gameMode[player] != modeNo_ver)) printFont(15 + 24 * player - 12 * maxPlay, 16, "OVER 20MIN", 2);
		if((time2[player] > 36000) && (gameMode[player] == modeNo_ver)) printFont(15 + 24 * player - 12 * maxPlay, 16, "OVER 10MIN", 2);
		...
		ExBltRect(86, 125 + 192 * player - 96 * maxPlay, 87, 72*(repdata[1 + player * 20]), 99 + (9*(repdata[0 + player * 20]+1)), 72, 9);
		...
		if(repdata[0 + player * 20] == modeNo_tom){	//TOMOYO
		...
		}else if(repdata[0 + player * 20] == modeNo_ver){	//VS
		...
		}else if(repdata[0 + player * 20] == modeNo_ace){	//ACE
		...
		}else if(repdata[0 + player * 20] == modeNo_mis){	//MISSION
			...
			getTime(repdata[5 + player * 20]);
			printSMALLFont(146 + 192 * player - 96 * maxPlay, 147, string[0], 0);
		}else if(repdata[0 + player * 20] == modeNo_std){	//SIMPLE
	...
	if(statc[player * 10 + 2] == 0){
		printFont(16+24 * player - 12 * maxPlay, 24, "A+C: SAVE", 0);
		...
		if( getPressState(player, 3) ) {
			...
			if(getPressState(player, 4) ) {
				if(getPressState(player,6)){
					if((time2[player] > 72000 - 36000 * (gameMode[player] == modeNo_ver)) || (!replay_save[player])){
						statc[player * 10 + 2] = 180;
					}else{
		...
		if(statc[player * 10 + 2] < 120){
			...
			statc[player * 10 + 4] = 0;
			statc[player * 10 + 5] = 0;
			if(gameMode[player] == modeNo_ver){	// VS
...
void saveReplayData(int pl, int number) {
	...
	if(gameMode[pl] == modeNo_ver){	// VSはフォーマットが一部異なる
	...
	saveBuf[207] = usePlayerdata;	// レベルアップボーナス #1.60c7g3
	...
	saveBuf[225] = ace_opt[pl];		// アナザーモード
	...
	saveBuf[254] = devil_opt[pl];		// DEVIL-
	...
	if((gameMode[pl] == modeNo_tom) && (stage[pl] < 100)) {
	...
	if(number <= 40)
		sprintf(string[0], "REPLAYC8/REPLAY%02d.SAV", number);
	else
...
int loadReplayData(int pl, int number) {
	...
	if(saveBuf[201] == modeNo_ver)	// VSはフォーマットが一部異なる
...
//	max = (saveBuf[200] + 3730) / 2 + 1;
	...
	usePlayerdata = saveBuf[207];		// レベルアップボーナス #1.60c7g3
	...
	ace_opt[pl] = saveBuf[225];		// アナザーモード
	...
	if(saveBuf[256] == 0) {
		p_shirase[pl] = (saveBuf[201] ==modeNo_dev);
...
int ReplaySelectProc(void) {
	...
	if(flag > 0) {
		...
		if(((gameMode[0] == modeNo_dev) && (!devil_opt[0])) || ((gameMode[0] == modeNo_ace) && (ace_opt[0] == 2))){
			PlaySE(18);//歓声
			gflash[0]=120;
		}
		return (2);
...
int ReplayBgmModeDecide(int pl,int mode,int nv,int dm,int eg){
	...
	}else if(mode==modeNo_dev){
		if(!dm){
			return 5;
		}else{
			return 6;
		}
	}else if(mode== modeNo_mas){
		if(eg==1){
			return 2;
		}else if(eg==4){
			return 4;
...
int getModeAddRep(void){
	if(saveBuf[201] == modeNo_beg) return saveBuf[259];
	else if((saveBuf[201] == modeNo_mas)) return saveBuf[232];
	else if(saveBuf[201] == modeNo_dev) return saveBuf[254];
	else if(saveBuf[201] == modeNo_tom) return (saveBuf[216] >= 27)+(saveBuf[216] >= 45)+(saveBuf[216] >= 69)+(saveBuf[216] >= 100);
	else if(saveBuf[201] == modeNo_ace) return saveBuf[225];
	else if(saveBuf[201] == modeNo_std){
		if(saveBuf[267]) return 4;
		else return saveBuf[260];
	}else if(saveBuf[201] == modeNo_ori) return saveBuf[296];
	else return 0;
}
void ReplaySelectInitial(void) {
	...
	for(i = 0; i < 40; i++) {
		...
		if(!loadReplayData(0, i + 1)) {
			...
			} else if(saveBuf[201] == modeNo_ace) {
				// ACE
			...
			} else if((saveBuf[201] != modeNo_mis) && (saveBuf[201] != modeNo_ver)){
				// 通常
				sprintf(string[10 + i], "%2d %7d  %4d  %4d  %s", i + 1, saveBuf[202], saveBuf[203], saveBuf[204], string[0]);
			} else
				sprintf(string[10 + i],"%2d                      %s ", i + 1,string[0]);
...
void ReplaySelect(void) {
	...
	printFont(3, 4, "SORRY.C8 CAN NOT PLAY C7 REPLAYFILE ", 5);
...
void ReplayDetail(int number) {
	...
	while(1) {
		...
		if(gameMode[0] == modeNo_beg) printFont(15, 4, "BEGINNER", 4);
		else if(gameMode[0] == modeNo_mas) printFont(15, 4, "MASTER", 1);
		else if(gameMode[0] == 2) printFont(15, 4, "20G", 5);
		else if(gameMode[0] == modeNo_dev) printFont(15, 4, "DEVIL", 2);
		else if(gameMode[0] == modeNo_ver) printFont(15, 4, "VERSUS", 7);
		else if(gameMode[0] == modeNo_tom) printFont(15, 4, "TOMOYO", 3);
		else if(gameMode[0] == modeNo_ace) printFont(15, 4, "ACE", 0);
		else if(gameMode[0] == modeNo_mis) printFont(15, 4, "MISSION", 8);
		else if(gameMode[0] == modeNo_std) printFont(15, 4, "SIMPLE", 9);
		else if(gameMode[0] == modeNo_ori) printFont(15, 4, "ORIGINAL", 0);
		else printFont(15, 4, "INVALID MODE", 0);
		
		if(death_plus[0]) printFont(21, 4, "DEATH+", 2);
		if(devil_opt[0]) printFont(28-(7*(!death_plus[0])), 4, "MINUS", 2);
		...
		if(gameMode[0] == modeNo_ace){
			if(ace_opt[0] == 1) printFont(18, 4, " ANOTHER", 0);
			if(ace_opt[0] == 2) printFont(18, 4, " HELL", 2);
		...
		printFont(1, 13, "PLAYER DATA  :", 0);
		if(usePlayerdata==1) printFont(15, 13, "e", 0);
		...
		if(gameMode[0] == modeNo_tom) {
			...
			if(saveBuf[240]) printFont(15, 23, "c", 0);
		...
		} else if((gameMode[0] != modeNo_mis)||(gameMode[0] != modeNo_std)) {
			// NORMAL
			if(gameMode[0] != modeNo_ace) {
			...
			if((p_shirase[0]) || ((devil_randrise) && (gameMode[0] == modeNo_dev) && (!devil_opt))) printFont(15, 20, "c", 0);
			else printFont(15, 20, "e", 0);
			
			printFont(1, 21, "RISE START  :", 0);
			if((devil_randrise) && (gameMode[0] == modeNo_dev) && (!devil_opt))
		...
		} else if(gameMode[0] == modeNo_mis){	// MISSION
			...
			printFont(15, 16, string[0], 0);
		}else if((gameMode[0] == modeNo_std) && (saveBuf[260] < 2)){	// STANDARD
	...
	//		段位
		if(gameMode[0] == modeNo_mas){
			printFont(20, 8, "GRADE TYPE :", 0);
			if(enable_grade[0] == 0) printFont(33, 8, "1", 0);
			else if(enable_grade[0] == 1) printFont(33, 8, "2+", 0);
			else if(enable_grade[0] == 2) printFont(33, 8, "2", 0);
			else if(enable_grade[0] == 3) printFont(33, 8, "3", 0);
		}
//		T-SPIN
		printFont(20, 9, "T-SPIN TYPE:", 0);
		if(tspin_type == 0) printFont(32, 9, "NO CHECK", 0);
		...
		if(gameMode[0] != modeNo_ver){
			...
			if(((death_plus[0]) || (hebo_plus[0])) && (max_hnext[0] >= 1))
				printFont(33, 10, "1", 0);
			else{ 
...
//		IRS
		if(disable_irs)
			printFont(20,12,"IRS DISABLE",0);
		else{
		printFont(20, 12, "IRS STYLE  :", 0);
		if(ace_irs == 0) printFont(33, 12, "HEBORIS", 0);
...
//TOMOYO
	if(repdata[0 + pl * 20] == modeNo_tom){
		repdata[4 + pl * 20] = tmpBuf[202];// stage
		repdata[5 + pl * 20] = tmpBuf[200];// time
//VERSUS
	}else if(repdata[0 + pl * 20] == modeNo_ver){
...
int getModeAddRep2(int *tmpBuf){
	if(tmpBuf[201] == modeNo_beg) return tmpBuf[259];
	else if((tmpBuf[201] == modeNo_mas)) return tmpBuf[232];
	else if(tmpBuf[201] == modeNo_dev) return tmpBuf[254];
	else if(tmpBuf[201] == modeNo_tom) return (tmpBuf[216] >= 27)+(tmpBuf[216] >= 45)+(tmpBuf[216] >= 69)+(tmpBuf[216] >= 100);
	else if(tmpBuf[201] == modeNo_ace) return tmpBuf[225];
	else if(tmpBuf[201] == modeNo_std){
		if(tmpBuf[267]) return 4;
		else return tmpBuf[260];
	}else if(tmpBuf[201] == modeNo_ori) return tmpBuf[296];
	else return 0;
}
