...
void tomoyoInitial(int player) {
	...
	if(tomoyo_opt[player]==3){
		lv[player] = sbak[15];	//FPBASICだけ戻す他のモードは初期化
	}
...
void statTomoyoNextStage(int player) {
	...
	if(tomoyo_opt[player] <= 1){
		Stage_update = Stage_RankingCheck(player, 6,tomoyo_opt[player],isWRule( rots[player]) );
	}
	...
	if(statc[player * 10 + 0] >= 300) {
		...
		if( (stage[player] == 19)&&(tomoyo_opt[player] == 0)) ltime[player] = ltime[player] + (60 * 60);	// 20面クリアで1分増加 #1.60c7l9
		...
		statc[player * 10 + 1] = 0;			// 
		if((tomoyo_opt[player] == 3) && (!t_training[player])){
		...
		if( (stage[player] > laststage[player]) && (!t_training[player])) {
			...
			end_f[player] = 1 + ((stage[player] >= 26)&&(tomoyo_opt[player] == 0));
		} else {
...
void statTomoyoSelect_FP(int player) {
	...
	if(statc[player * 10] == 0){	//最初
		...
		if( getPressState(player, 2) ) {
			...
			FP_bonus[player] = 1000 * (((start_stage[player]-100) / 4) + 1);
			// ステージデータ読み込み #1.60c7l9
			loadTomoyoStage(player, start_stage[player]);
		}
