...
void statDMove(int player) {
	int		i, bak, bottom, move, don, flash, kosa, rolling,k,synchro,y_hosei,hosei_type,lockT;
...
//	statc[player * 10 + 4] = 1; //横先行入力は常に可能
	...
	if(!statc[player * 10 + 2]) {
		...
		if(move && (statc[player * 10 + 4])) {
			...
			if((mpc[player] == 1) || (mpc[player] >= waitt[player]) || ((isremote[player]) && (gameMode[player] != modeNo_ver)) || (mpc[player] >= 8))
...
//		statc[player * 10 + 4] = 1; 				 // 最初の1フレームは移動させないゾ
		...
		if((judgeBlock(player, bx[player], by[player] + 1, blk[player], rt[player]) != 0)) {
			...
			if((bk[player] > lockT) || (bk_D[player] > 30)) {
				...
				if( (!harddrop[player])) goto lockflash;	// SUPERなSKIP
				return;
			}
		} else {
		...
		while(bs[player] > 60) {
			...
			if(judgeBlock(player, bx[player], by[player] + 1, blk[player], rt[player])) {
				...
				if( getPressState(player, 1-(1*(isUDreverse[player]))) && (!down_flag[player]) && ((!move || nanamedown) )) {
				...
				// 上入れ即接着 #1.60c7r3
				if(getPressState(player , 0+(1*(isUDreverse[player])))
					&&(!up_flag[player]) && (heboGB[player]==0) 
					&& (statc[player * 10 + 4]) && (!move || nanamedown)) {
					PlaySE(3);
					kickc[player] = 0;
					harddrop[player]=1;
...
//		UpLineBlockJudge(player);
		...
		if(statc[player * 10 + 2] > 1) {	// 接着時の灰色化を1フレ短縮 #1.60c7k8
			...
			if((tspin_flag[player]) && (blk[player] == 4)&& ((tspin_type>=2) || (gameMode[player] == modeNo_mis)) && (statc[player * 10 + 5] > 0) && (isTSpin(player) >= 3) ) {
				...
				last_tspin[player] = 0;
				tspin_flag[player] = 2;
			} else if((tspin_type==2) || (gameMode[player] == modeNo_mis)){//3コーナーだけの時は逆に下ろす
				tspin_flag[player] = 0;
			}
