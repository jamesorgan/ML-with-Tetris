...
void ConfigMenu() {
	...
	loop {
		...
		if(stat[0] == 0){
			...
			printFont(2, 25, "USE PLAYDATA:", (statc[0] == 15) * fontc[rots[0]]);
...
//			if(statc[1]) {	// 表示更新
				...
				if(ncfg[44] == 0) sprintf(string[0], "MIDI");
				else if(ncfg[44] == 1) sprintf(string[0], "WAVE");
				else if(ncfg[44] == 2) sprintf(string[0], "MP3");
				...
				// ユーザーデータ
				if(ncfg[54]) sprintf(string[0], "ON");
				else sprintf(string[0], "OFF");
...
//				m = getPushState(pl, 1) - getPushState(pl, 0);
				...
				if(getPushState(pl, 7)) {
					...
					if(m) {
						...
						if(statc[0] == 1){
							ncfg[0] = (ncfg[0] & 0x100) + ((ncfg[0] % 0x100) + 7 + m) % 7;	// screenMode
							need_reset = 1;
						}
						else if(statc[0] == 2) {
							...
							if(ncfg[44] < 0) ncfg[44] = 2;
							if(ncfg[44] > 2) ncfg[44] = 0;
							
							if(bgm) need_reset = 1;
						}
						else if(statc[0] == 13) ncfg[52] = !ncfg[52];		// w_reverse
						else if(statc[0] == 14) ncfg[53] = !ncfg[53];		// downtype
						else if(statc[0] == 15) ncfg[54] = !ncfg[54];		// プレイヤーデータ
...
void GameOption() {
	...
	loop {
		...
		sprintf(string[0],"NAVIGATION        : %d");
		if(navigation == 0) sprintf(string[0], "OFF");
			else sprintf(string[0], "ON");
		...
		if( getPressState(0, 2) ) {
			...
			if(cursor == 1) {
				enable_grade--;
				if(enable_grade < 0) enable_grade = 3;
			}			
			//IRS
			if(cursor == 2) {
				ace_irs--;
		...
		if( getPressState(0, 3) ) {
			...
			if(cursor == 1) {
				enable_grade++;
				if(enable_grade > 3) enable_grade = 0;
			}
			// IRS
			if(cursor == 2) {
				ace_irs++;
