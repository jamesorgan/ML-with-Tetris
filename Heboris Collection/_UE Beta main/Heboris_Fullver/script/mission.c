...
void viewMission() {
	...
	if((mission_type[c_mission] >= 30) && (mission_type[c_mission] <= 33)||(mission_type[c_mission] == 38)){
		if(mission_opt_3[c_mission]<=6){
			...
			if ( getDrawRate() == 1 ){
				if(!english){
					drawBlockFast(2 + 25 * maxPlay, 20 + isWRule( rots[0]) * (mission_opt_3[c_mission] != 0) + 
					...
					(mission_type[c_mission] == 33) + isWRule( rots[0]) * (mission_opt_3[c_mission] != 0) + 
					((mission_opt_3[c_mission] == 0) && (rots[0] != 8)), mission_opt_3[c_mission], 0, c_tmp, 0, 1, 0, 0, 0);
				}
			}else{
				if(!english){
					drawBlockFast(2 + 25 * maxPlay, 16 + isWRule( rots[0]) * (mission_opt_3[c_mission] != 0) + 
			...
			if ( getDrawRate() == 1 ){
				if(!english){
					...
					drawBlockFast(6 + 25 * maxPlay, 20 + isWRule( rots[0]) * (mission_opt_3[c_mission] != 0) + 
				...
				if(!english){
					...
					drawBlockFast(6 + 25 * maxPlay, 16 + isWRule( rots[0]) * (mission_opt_3[c_mission] != 0) + 
...
void missionNormUp(int lines) {
	...
	// OOBAKA
	if( (mission_type[c_mission] == 29) && (stat[0] == 6) && ((timeOn[0])) ) {
		c_norm[0]++;
	}
...
void setEraserLines() {
	...
	for(i = 0; i <= mission_opt_3[c_mission]; i++) {
		...
		if(min == max) {
			// 位置固定
			tmp = min+1;
		}else if((i >= mission_norm[c_mission] - c_norm[0] - 1) && (mission_norm[c_mission] > 1) && (line[min + 1] != 1) ){
			// 最後の1本
			tmp = min+1;
		}else{
			do {
...
statc[0 * 10 + 6] = mission_opt_3[c_mission];
	if(mission_opt_1[c_mission] == mission_opt_2[c_mission]){	//ステージが1つだけ
		...
		do {
			tmp = rand(mission_opt_2[c_mission] + 1,0);
	...
	if((tomoyo_rise[0] == 0) && (mission_opt_3[c_mission] != 0)) randPlatinum(0, mission_opt_3[c_mission]);
}
