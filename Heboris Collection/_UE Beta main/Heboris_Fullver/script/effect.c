...
void objectStatBlock(int no) {
	...
	//breaktypeを3にするとBEGINNERのみ旧エフェクト、それ以外新エフェクト #1.60c7l1
	}else if( (gameMode[objp[no]] == modeNo_ori) || (breaktype == 0) || ((breaktype == 3) && (gameMode[objp[no]] == modeNo_beg) && (!novice_mode[objp[no]])) ) {
		...
		if( objj[no] >= 9 ) {
			...
			if((obja[no] < 32) && (gameMode[objp[no]] == modeNo_tom) && (tomoyo_opt[objp[no]] == 0 ))
...
void objectStatBlock2(int no) {
	...
	//breaktypeを3にするとBEGINNERのみ旧エフェクト、それ以外新エフェクト #1.60c7l1
		...
		if( objj[no] >= 9 ) {
			// プラチナブロック #1.60c7m9
			objectplatina_erase(no);
			if((obja[no] < 32) && (gameMode[objp[no]] == modeNo_tom) && (stage[objp[no]] < 100))
				ExBltRect(3, objx[no] / 100, (objy[no]-(obja[no]*30)) / 100, 212+(12*(objj[no]-10)), 64, 12, 7);
			objy[no] = objy[no] - 20;
			if(obja[no] >= 59) obj[no] = 0;
		} else {
...
void objectAllCrear(int no) {
	...
	if(gameMode[objp[no]] <= modeNo_dev)
...
void objectItemWarning(int no){
	...
	if(((objj[no]==2) && (IsBigStart[objp[no]])) || ((objj[no]==36) && (gameMode[objp[no]] != modeNo_ver)))
...
void objectdelfield(int no) {
	...
	if(!istimestop[objp[no]]){
		obja[no]++;
		if((wait2[objp[no]] <= 12) && ((timeOn[objp[no]]) || (ending[objp[no]] == 2) || (objc[no])))
			obja[no] = obja[no] + 1;
	}
