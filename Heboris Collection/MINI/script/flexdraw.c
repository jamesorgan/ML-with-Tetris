// •`‰æ”{—¦‚ð‰Â•Ï‚Å‚«‚é‚æ‚¤‚É‚È‚Á‚½ŠÖ”ŒQB
// ’Êí‚Ì•`‰æŠÖ”‚Ì“ª‚É Ex ‚ð‚Â‚¯‚Ü‚·B
...
{
	TextLayerOn(n, x * ((screenMode >= 2) + 1), y * ((screenMode >= 2) + 1));
}

void ExTextMove(int n, int x, int y)
{
	TextMove(n, x * ((screenMode >= 2) + 1), y * ((screenMode >= 2) + 1));
}

void ExTextSize(int n, int size)
{
	TextSize(n, size * ((screenMode >= 2) + 1));
}

void ExTextHeight(int n, int size)
{
	TextHeight(n, size * ((screenMode >= 2) + 1));
}
...
void ExSaveBitmap(str *nameStr, int pno, int x, int y, int sx, int sy)
{
	SaveBitmap(nameStr, pno, x, y, sx * zoomRate, sy * zoomRate);
}
